...
function tree_out=build_btree_parent3(parent,p,eh,ev,np)
%np表示上一个节点
global node_number
...
%% 判断是否到达终点
        ...
        elseif np<size_input
            ...
            %按照概率大小创建分支
            if uc(np)==1 %不正常点
                tree_cell(assigned_nn).left=build_btree_parent3(assigned_nn,0.8,eh,ev,np);
                tree_cell(assigned_nn).right=build_btree_parent3(assigned_nn,0.2,eh,ev,np);
            elseif uc(np)==0  %正常点
                tree_cell(assigned_nn).left=build_btree_parent3(assigned_nn,1,eh,ev,np);
            end
        end
