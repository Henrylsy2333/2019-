% This is the program of the third question about the first data file.
...
%%
clc;clear;
%position_raw = xlsread('附件1：数据集1-终稿.xlsx','B3:F615');
%save C:\Users\Lenovo\Desktop\2019数学建模\Matlab\Raw_Pos13.mat
%save position_raw2
global position_raw
...
vtcpos=position_raw(find(mark==1),[1:4,6]); % 垂直误差校正点
hrzpos=position_raw(find(mark==0),[1:4,6]); % 水平误差校正点
% 垂直校准点集
vx=vtcpos(:,1);
vy=vtcpos(:,2);
vz=vtcpos(:,3);
...
theta=30;
delta=0.001;
rref = 2*10^4; %搜索半径，和密度有关
%% 蚁群算法相关参数
N=50; %跑N条轨迹
Q=100; %全值
G=20; %迭代次数
tau=1.*(ones(613,613)-eye(613));%存储信息素的矩阵,先给一个初值（切忌！不可都给0）
%%
for j=1:G
    PointIndex = zeros(40,N); %每一列用来存放，第n个蚂蚁经过的路径
    ...
    while n<=N  %多个蚂蚁分别跑，一遍跑完统一更新信息素
        ...
        %关于轨迹和点的变量
        ip=2; %index of point
        PointRecord =zeros(40,6);%假设每条轨迹最多有50个点
        PointRecord(1,:)=[x0;0;1;0]';
        %% 蚁群算法参数
        alpha=3; %信息素的重要性程度
        beta=1;  %启发式函数的重要性程度   注：可以采用变参数的思想，一开始以启发式函数为主，然后以信息素为主
        current_i=1;
        target_j=1;
        %%        
