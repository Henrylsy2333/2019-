% This is the program of the third question about the Second data file.
...
for j=1:G
    ...
    while n<=N  %多个蚂蚁分别跑，一遍跑完统一更新信息素
        ...
        %% 蚁群算法参数
        alpha=10; %信息素的重要性程度
        beta=3;  %启发式函数的重要性程度   注：可以采用变参数的思想，一开始以启发式函数为主，然后以信息素为主
        current_i=1;
        target_j=1;
        %%
...
%             rdvct=dcos; %random vector
            ...
            %精英法
            %Filter = 0.5;
            %randd=fix(Filter*length(randd)) %为了增加收敛性
            %构造非线性函数法
...
%         end
        ...
        L(n)=ComputeSize2(PointRecordA(:,5));
        PointIndex(:,n)=PointRecord(:,5);      
        Lt = L;  
        n = n+1;
%             In = PointRecord(:,5);
%             In1 = In(1:find(In,1,'last'));
%             Lt(n) = CheckPath(In1);
%             n=n+1;
    end
    %% 更新信息素
